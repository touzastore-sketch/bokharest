...
export const GalleryModal: React.FC = () => {
  ...
  const currentImage = GALLERY_IMAGES[selectedGalleryIndex] || GALLERY_IMAGES[0];
    ...
    >
      ...
      <div className="relative z-20 flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-white/10 bg-black/60 backdrop-blur-md">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={handleClose}
            className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-all active:scale-95 focus:outline-none cursor-pointer"
      ...
      <div className="relative flex-1 flex items-center justify-center p-2 sm:p-6 overflow-hidden">
        ...
        <button
          id="gallery-prev-btn"
          onClick={handlePrev}
          className="absolute left-2 sm:left-6 z-20 w-11 h-11 sm:w-13 sm:h-13 rounded-full bg-black/60 hover:bg-black/90 border border-white/20 text-white flex items-center justify-center backdrop-blur-md transition-all active:scale-95 shadow-2xl focus:outline-none cursor-pointer group"
        ...
        <div className="relative max-w-4xl max-h-[62vh] sm:max-h-[68vh] w-full h-full flex items-center justify-center">
          ...
          <img
            ...
            onError={(e) => {
              // Direct permanent hosted fallback
              (e.currentTarget as HTMLImageElement).src = currentImage.url;
            }}
        ...
        <button
          id="gallery-next-btn"
          onClick={handleNext}
          className="absolute right-2 sm:right-6 z-20 w-11 h-11 sm:w-13 sm:h-13 rounded-full bg-black/60 hover:bg-black/90 border border-white/20 text-white flex items-center justify-center backdrop-blur-md transition-all active:scale-95 shadow-2xl focus:outline-none cursor-pointer group"
